...
\hypersetup{
  ...
  linkcolor=tsgreen,
  urlcolor=tsgreen,
  pdftitle={Memoria TurismoSemántico},
...
\begin{tabular}{rl}
\textbf{Autoría:} & \autor\\
\textbf{Repositorio:} & \href{https://github.com/SergioAlvesMar5/ProjectoFinal_MSW_TurismoSemantico}{\texttt{ProjectoFinal\_MSW\_TurismoSemantico}}\\
...
Dos fuentes abiertas & Wikidata, OpenStreetMap/Nominatim/Overpass y Open-Meteo; Anthropic es opcional y no sustituye a las fuentes abiertas.\\
...
Cuatro funcionalidades o más & Mapa, POIs por ciudad, búsqueda semántica, chatbot RAG, grafo RDF, SPARQL y descarga Turtle.\\
...
\begin{enumerate}
  ...
  \item \textbf{Indexación semántica}: se construye un texto indexable con nombre, tipo, ciudad y descripción. Se generan embeddings con \texttt{paraphrase-multilingual-MiniLM-L12-v2}; si el entorno no dispone de \texttt{sentence-transformers}, ChromaDB o del modelo descargado, se activa un fallback TF-IDF/Jaccard.
\end{enumerate}

El código añade resiliencia práctica: reintentos y timeouts en APIs externas, varios endpoints Overpass alternativos, conversión de caché antigua a UTF-8, carga OSM en hilo de fondo para no bloquear la interfaz, y degradación controlada cuando faltan spaCy, ChromaDB, el modelo de embeddings o una clave LLM.
...
La API expone los endpoints principales: \texttt{/api/cargar}, \texttt{/api/estado}, \texttt{/api/destinos}, \texttt{/api/buscar}, \texttt{/api/chat}, \texttt{/api/grafo}, \texttt{/api/sparql}, \texttt{/api/shacl}, \texttt{/api/ttl}, \texttt{/api/clima} y \texttt{/api/overpass}. \texttt{/api/shacl?validar=1} permite comprobar el grafo frente a las shapes. Esta separación permite que cada pantalla consuma solo los datos necesarios.
...
\foreach \x/\y in {-2.35/1.0, -2.05/-1.05, 1.85/0.95, 2.75/-1.25} {
