...
\centering
\small
\begin{tabularx}{\textwidth}{p{2.8cm}p{2.6cm}X p{2.1cm}}
\toprule
...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners, fill=tslight, align=center, minimum width=2.7cm, minimum height=0.82cm},
  store/.style={draw, cylinder, shape border rotate=90, aspect=0.25, fill=white, align=center, minimum width=2.2cm, minimum height=0.9cm},
  arrow/.style={-{Latex[length=3.2mm,width=2.3mm]}, line width=1.15pt, draw=tsgreen!85!black}
]
\node[box] (ui) {Frontend\\HTML/CSS/JS\\Leaflet + D3};
\node[box, right=of ui] (api) {Flask API\\\texttt{app.py}};
\node[box, above right=0.25cm and 1.7cm of api] (wiki) {Wikidata\\SPARQL};
\node[box, right=of api] (pipe) {Pipeline\\limpieza + fusión};
\node[box, below right=0.25cm and 1.7cm of api] (osm) {OSM +\\Open-Meteo};
\node[store, right=of pipe] (json) {JSON\\caché};
\node[store, above=of json] (ttl) {RDF/OWL\\Turtle};
\node[store, below=of json] (chroma) {ChromaDB\\vectores};
\node[box, right=of json] (features) {Mapa, búsqueda,\\RAG, SPARQL, grafo};
\path[arrow] (ui.east) -- node[above]{REST} (api.west);
\path[arrow] (api.north east) -- (wiki.west);
\path[arrow] (api.south east) -- (osm.west);
\path[arrow] (wiki.east) -- (pipe.north west);
\path[arrow] (osm.east) -- (pipe.south west);
\path[arrow] (pipe.east) -- (json.west);
\path[arrow] (pipe.north east) -- (ttl.west);
\path[arrow] (pipe.south east) -- (chroma.west);
\path[arrow] (json.east) -- (features.west);
\path[arrow] (ttl.east) -- (features.north west);
\path[arrow] (chroma.east) -- (features.south west);
\path[arrow] (features.south) |- (ui.south);
\end{tikzpicture}}
...
\path[arrow] (map.south) |- (graph.east);
\end{tikzpicture}}
